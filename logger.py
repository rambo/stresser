...
class logger(zmqdecorators.service):
    ...
    def init_db(self):
        ...
        self.cursor.execute("CREATE TABLE log (clientid TEXT NOT NULL, time TIMESTAMP DATETIME DEFAULT(STRFTIME('%Y-%m-%d %H:%M:%f', 'NOW')), url TEXT, action TEXT, argsjson TEXT, httpstatus INTEGER, walltime INTEGER, ttfb INTEGER, ttlb INTEGER, ttrdy INTEGER, perfjson TEXT);")
        self.connection.commit()

    @zmqdecorators.method()
    def log(self, resp, timestamp, url, action, argsjson, httpstatus, walltime, ttfb, ttlb, ttrdy, perfjson):
        self.cursor.execute("INSERT INTO log (clientid, time, url, action, argsjson, httpstatus, walltime, ttfb, ttlb, ttrdy, perfjson) VALUES (?,?,?,?,?,?,?,?,?,?,?);", (resp.client_id, timestamp, url, action, argsjson, int(httpstatus), int(walltime), int(ttfb), int(ttlb), int(ttrdy), perfjson))
        self.connection.commit()
